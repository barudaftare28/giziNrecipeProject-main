{{-- resources/views/customer/recipes/index.blade.php --}}
@extends('layouts.customer')

@section('title', 'Resep Kamu | NutriRecipe')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="mt-1 text-sm text-neutral-400">
                    Jelajahi resep official dan resep buatan komunitas. Klik kartu untuk melihat detail, langkah, dan
                    informasi gizinya.
                </p>
            </div>
        </div>
        ...
        <div class="bg-white border border-neutral-100 rounded-sm p-4">
            <div class="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                <form action="{{ route('customer.recipes.index') }}" method="GET" class="flex-1 flex gap-2">
                    @if (request('type'))
        ...
        {{-- Grid resep --}}
        @if ($recipes->isEmpty())
            <div class="bg-white border border-neutral-100 rounded-sm {{-- shadow-sm --}} p-6 text-sm text-neutral-600">
                <p class="font-medium">Belum ada resep yang diposting.</p>
                <p class="mt-1 text-neutral-500">
                    Coba tambahkan resep pertamamu dengan menekan tombol
                    <span class="font-semibold">"Tambah Resep Kamu"</span> di kanan atas.
                </p>
            </div>
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($recipes as $recipe)
                    <a href="{{ route('customer.recipes.show', $recipe->id) }}"
                        class="group bg-white border border-neutral-100 rounded-sm {{-- shadow-sm hover:shadow-md transition-shadow duration-200 --}} overflow-hidden flex flex-col">
                        {{-- Foto --}}
                        ...
                        <div class="flex-1 p-4 flex flex-col gap-2">
                            ...
                            <p class="text-xs text-neutral-500">
                                Oleh:
                                @if ($recipe->is_official)
                                    <span class="font-medium text-neutral-700">Pakar Gizi
                                        ({{ $recipe->nutritionist }})
                                    </span>
                                @else
                                    <span class="font-medium text-neutral-700">{{ $recipe->user->name ?? 'User' }}</span>
                                @endif
                            </p>
